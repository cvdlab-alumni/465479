...
/*-------------------------------------------------------------
Function that makes a grid of roads.
-------------------------------------------------------------*/
function make_roads(dim){
	var i;
	var j;
	...
	for(i = 0; i<dim-1; i++){
		for(j = 0; j<dim-1; j++){
			var rd_2 =  CUBOID([1,0.2,0.1]);
			var color = [0, 0, 0];
			rd = COLOR(color)(rd_2);
			rd = T([0,1,2])([dim+i+0.5,dim+j+0.5,5])(rd);
			roads_array.push(rd)
			//DRAW(rd);
			
			var rd_1 =  CUBOID([0.2,1,0.1]);
			var color = [0, 0, 0];
			rd_1 = COLOR(color)(rd_1);
			rd_1 = T([0,1,2])([dim+i+0.5,dim+j+0.5,5])(rd_1);
			roads_array.push(rd_1)
			//DRAW(rd_1);
		}				
		var rd_2 =  CUBOID([1,0.2,0.1]);
		var color = [0, 0, 0];
	...
	var rd_1 =  CUBOID([0.2,dim-1+0.2,0.1]);
	var color = [0, 0, 0];
	rd = COLOR(color)(rd_1);
	rd = T([0,1,2])([dim+i+0.5,j+1.5,5])(rd);
	roads_array.push(rd)
	//DRAW(rd);
	
	return STRUCT(roads_array);
}
...
function create_house(dims,trasl){
	...
	hs = COLOR(color)(hs);
	hs = T([0,1,2])([trasl[0]+0.5,trasl[1]+0.5,trasl[2]])(hs);
	
	var rd =  CUBOID([0.4,0.1,0.2]);
	var color = [0, 0, 0];
	rd = COLOR(color)(rd);
	rd = T([0,1,2])([trasl[0],trasl[1],trasl[2]])(rd);
	return hs;
}
